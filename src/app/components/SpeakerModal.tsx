...
export default function SpeakerModal({ speakerId, onClose }: SpeakerModalProps) {
  ...
  return (
    <>
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
        <div className={`bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto ${selectedSession ? 'mr-4' : ''}`}>
          <div className="p-6">
            {/* Header */}
            ...
            <div className="mb-6">
              <div className="mb-3">
                ...
                <span className="text-gray-900">{speaker.field}</span>
              </div>
              <div>
                ...
                <div className="flex flex-wrap gap-2 mt-1">
                  {speaker.position.map((position, index) => (
                    <span
                      key={index}
                      className={`text-sm px-3 py-1 rounded-full ${getPositionColor(position)}`}
                    >
                      {position}
                    </span>
                  ))}
